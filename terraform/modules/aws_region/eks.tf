...
resource "aws_iam_role" "aws_load_balancer_controller" {
  name = "eks-aws-load-balancer-controller-role-${local.short_uuid}"
...
resource "aws_iam_policy" "aws_load_balancer_controller" {
  name   = "AWSLoadBalancerControllerIAMPolicy-${local.short_uuid}"
  policy = <<EOT
{
    "Version": "2012-10-17",
    "Statement": [
...
module "eks" {
  ...
  subnet_ids = [aws_subnet.shh_subnet1.id, aws_subnet.shh_subnet2.id]

  cluster_tags = {
    "karpenter.sh/discovery" = var.eks_cluster_name
    "env"                    = var.eks_cluster_name
  }
